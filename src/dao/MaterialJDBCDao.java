...
public class MaterialJDBCDao implements MaterialDao {
    ...
    public Vector carregarLotesMaterial(int codMaterial) throws SQLException {
        ...
        String sql;
        PreparedStatement ps;
        sql = "SELECT entrada.cod_lote,  COALESCE(entrada.val_entrada - saida.val_saida,0) AS total " + "FROM (SELECT lo.cod_lote as cod_lote, " + "sum(lo.qnt_entrada) as val_entrada " + "FROM lote lo " + "WHERE lo.cod_material = " + codMaterial + "GROUP BY lo.cod_lote " + "ORDER BY lo.cod_lote) entrada, " + "(SELECT lo.cod_lote as cod_lote, " + "COALESCE(sum(sm.qnt_saida),0) as val_saida " + "FROM saida_material sm " + "RIGHT OUTER JOIN lote lo ON sm.cod_lote = lo.cod_lote " + "WHERE lo.cod_material = " + codMaterial + "GROUP BY lo.cod_lote  " + "ORDER BY lo.cod_lote) saida " + "WHERE entrada.cod_lote = saida.cod_lote AND " + "COALESCE(entrada.val_entrada - saida.val_saida,0) > 0;";
        ps = conexao.prepareStatement(sql);
        ResultSet res = ps.executeQuery();
    ...
    public void alterarEstoqueMaterial(int id, Double soma) throws SQLException {
        try {
            conexao = FabricaConexao.obterConexao("JDBC");
            conexao.setAutoCommit(false);
            String sql = " UPDATE material SET " + " estoque = estoque - ? " + " WHERE cod_material = " + id;
            PreparedStatement ps = conexao.prepareStatement(sql);
            ps.setDouble(1, soma);
            ps.executeUpdate();
            conexao.commit();
