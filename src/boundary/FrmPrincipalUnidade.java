...
public class FrmPrincipalUnidade extends javax.swing.JFrame {

    ControladoraUnidade controladora;
    Vector responsavel;

    public FrmPrincipalUnidade(Vector responsavel, String servidor) {
        initComponents();
        controladora = new ControladoraUnidade(servidor);
    ...
    private void preencherTabela() {
        ...
        DefaultTableModel modelo = (DefaultTableModel) this.tbUnidades.getModel();
    ...
    private void limparTabela() {
        DefaultTableModel modelo = (DefaultTableModel) this.tbUnidades.getModel();
        int numLinhas = tbUnidades.getRowCount();
        for (int i = 0; i < numLinhas; i++) {
            modelo.removeRow(0);
        }
    }
    ...
    private Vector criarLinhaSelecao() {
        ...
        for (int i = 0; i < 2; i++) {
            linha.add(this.tbUnidades.getModel().getValueAt(this.tbUnidades.getSelectedRow(), i));
        }
        return linha;
    }
    ...
    private void initComponents() {
        ...
        tbUnidades.getTableHeader().setReorderingAllowed(false);
        jScrollPane1.setViewportView(tbUnidades);
        tbUnidades.getColumnModel().getColumn(0).setMinWidth(50);
        tbUnidades.getColumnModel().getColumn(0).setMaxWidth(50);
    ...
    private void btInserirMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btInserirMouseClicked
        JDialog janela = new FrmInserirUnidade(controladora);
    ...
    private void btAlterarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btAlterarMouseClicked
        int linhaSelecionada = this.tbUnidades.getSelectedRow();
        if (linhaSelecionada < 0) {
            JOptionPane.showMessageDialog(null, "Você deve selecionar uma Unidade !", "Atenção", JOptionPane.WARNING_MESSAGE);
        } else {
            JDialog janela = new FrmAlterarUnidade(controladora, criarLinhaSelecao(), cbSatus.getSelectedItem().toString());
            janela.setModal(true);
            janela.setVisible(true);
            limparTabela();
            preencherTabela();
    ...
    private void btAtivarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btAtivarMouseClicked
        ...
        if (linhaSelecionada < 0) {
            ...
            JDialog janela = new FrmAtivarUnidade(controladora, criarLinhaSelecao(), responsavel);
            janela.setModal(true);
            janela.setVisible(true);
            limparTabela();
            preencherTabela();
    ...
    private void btDesativarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btDesativarMouseClicked
        ...
        if (linhaSelecionada < 0) {
            ...
            JDialog janela = new FrmDesativarUnidade(controladora, criarLinhaSelecao(), responsavel);
            janela.setModal(true);
            janela.setVisible(true);
            limparTabela();
            preencherTabela();
