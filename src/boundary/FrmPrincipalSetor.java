...
public class FrmPrincipalSetor extends javax.swing.JFrame {
    ...
    private void preencherTabela() {
        ...
        DefaultTableModel modelo = (DefaultTableModel) this.tbSetores.getModel();
    ...
    private void limparTabela() {
        DefaultTableModel modelo = (DefaultTableModel) this.tbSetores.getModel();
        int numLinhas = tbSetores.getRowCount();
        for (int i = 0; i < numLinhas; i++) {
            modelo.removeRow(0);
        }
    }
    ...
    private Vector criarLinhaSelecao() {
        ...
        for (int i = 0; i < 2; i++) {
            linha.add(this.tbSetores.getModel().getValueAt(this.tbSetores.getSelectedRow(), i));
        }
        return linha;
    }
    ...
    private void initComponents() {
        ...
        tbSetores.getColumnModel().getColumn(0).setMinWidth(50);
        tbSetores.getColumnModel().getColumn(0).setMaxWidth(50);
        tbSetores.getColumnModel().getColumn(1).setResizable(false);
    ...
    private void btInserirMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btInserirMouseClicked
        JDialog janela = new FrmInserirSetor(controladora);
    ...
    private void btAlterarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btAlterarMouseClicked
        int linhaSelecionada = this.tbSetores.getSelectedRow();
        if (linhaSelecionada < 0) {
            JOptionPane.showMessageDialog(null, "Você deve selecionar uma Setor !", "Atenção", JOptionPane.WARNING_MESSAGE);
        } else {
            JDialog janela = new FrmAlterarSetor(controladora, criarLinhaSelecao(), cbSatus.getSelectedItem().toString());
            janela.setModal(true);
            janela.setVisible(true);
            limparTabela();
            preencherTabela();
    ...
    private void btAtivarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btAtivarMouseClicked
        ...
        if (linhaSelecionada < 0) {
            ...
            JDialog janela = new FrmAtivarSetor(controladora, criarLinhaSelecao(), responsavel);
            janela.setModal(true);
            janela.setVisible(true);
            limparTabela();
            preencherTabela();
    ...
    private void btDesativarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btDesativarMouseClicked
        ...
        if (linhaSelecionada < 0) {
            ...
            JDialog janela = new FrmDesativarSetor(controladora, criarLinhaSelecao(), responsavel);
            janela.setModal(true);
            janela.setVisible(true);
            limparTabela();
            preencherTabela();
